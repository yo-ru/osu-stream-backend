...
async def dl_download_get():
    ...
    if not pack_id or not filename or not device_id or not preview:
        return "fail: missing required arguments", 400

    params = urlparse.urlencode(
        {"pack": pack_id, "filename": filename, "id": device_id, "preview": preview},
    )
    return redirect(
        f"https://www.osustream.com/dl/download2.php{f"?{params}" if params else ""}",
        307,
    )
